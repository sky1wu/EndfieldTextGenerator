...
_wubi_98_dict: Optional[dict[str, list[str]]] = None
...
def _load_json() -> dict[str, list[str]]:
    """从 JSON 文件加载 98 版五笔码表。更新：刃下狼血"""
    ref = resources.files('pywubi').joinpath('data/wubi_98.json')
    with resources.as_file(ref) as path:
        with open(path, 'r', encoding='utf-8') as f:
            return json.load(f)

def get_wubi_dict_98() -> dict[str, list[str]]:
    """
    五笔98字典调用，更新：刃下狼血
    """
    global _wubi_98_dict
    if _wubi_98_dict is None:
        _wubi_98_dict = _load_json()
    return _wubi_98_dict
